...
async function main() {
    ...
    // await (await c1.connect(dev2).setApprovalForAll(pp.address, true)).wait()
    // await (await c2.connect(dev2).setApprovalForAll(pp.address, true)).wait()

    // await (await c1.connect(dev3).setApprovalForAll(pp.address, true)).wait()
    // await (await c2.connect(dev3).setApprovalForAll(pp.address, true)).wait()


    // await (await sa.grantRole(
    //     await sa.AUCTION_CREATOR_ROLE(),
    //     dev2.address
    // )).wait()
    ...
    const block = await ethers.provider.getBlock(initialBlocknumber)
    console.log(block.timestamp)

    // await (await sa.connect(dev3).create(
    //     c1.address,
    //     '1',
    //     '1000000000000',
    //     '1660348800'
    ...
    // await (await sa.connect(dev2).bid(
    //     c1.address,
    //     '1',
    //     '10000000000000',
    ...
    // await (await sa.connect(dev2).cancelAuctionAdmin(
    //     c2.address,
    //     '1'
    // )).wait()
    ...
    // await (await sa.connect(dev2).create(
    //     c1.address,
    //     '5',
    //     '1000000000000',
    //     '1660694400'
    ...
    await (await sa.connect(dev2).settle(
        c1.address,
        '1'
    )).wait()
}
